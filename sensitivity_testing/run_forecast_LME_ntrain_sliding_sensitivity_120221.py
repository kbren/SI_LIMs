...
# limvars_list = [['sic'],['sic','tas'],['sic','psl'],
#                 ['sic','zg'],['sic','tos'], ['sic','sit'],
#                 ['tas','psl','zg','tos','sit','sic']]
...
# yrend_list = [1650, 1550, 1450, 1350, 1250, 1150, 1050, 950]
nyrstrain = [100]
#yrend_list = [1050, 950]
nloop = [1]
...
save_folder = '/home/disk/kalman2/mkb22/SI_LIMs/sensitivity_testing/ntrain/'+folder_add
...
ntrunc = ntrunc_list[0]
exp_setup['ntrunc'] = ntrunc
exp_setup['nmodes_sic'] = nmodes_sic_list[0]

for y,ntrain in enumerate(nyrstrain):
    print('ntrain = '+str(ntrain))
    
    # LME settings (out of sample): 
    exp_setup['nyearsvalid'] = 100
    exp_setup['nyearstrain'] = ntrain
    exp_setup['nyearstot'] = 1155
    exp_setup['nyears_startvalid'] = 901*12
    
        # #LME settings (in sample): 
#     exp_setup['nyearsvalid'] = yrvalid
#     exp_setup['nyearstrain'] = (yrend-850)
#     exp_setup['nyearstot'] = 1155

#     startyr = 801
#     if (exp_setup['nyearstot']-startyr)/yrvalid >1: 
#         nloop = int(np.floor((1001-startyr)/yrvalid))
#     else: 
#         nloop = 1
...
#         startyr = startyr+i*ntrain
        
        exp_setup['nyears_starttrain'] = startyr
...
#         mod_sic_filename = ('_ntrunc'+str(nmodes_sic_list[0])+'_002_monthall_'+train_dsource+'_latcutoff_'+
#                             str(exp_setup['lat_cutoff'])+'_wtTrue_dtTrue_ntrain_850_'+str(yrend)+'_20211014.pkl')

#         exp_setup['mod_filename'] = mod_filename
#         exp_setup['mod_sic_filename'] = mod_sic_filename
#         if i >0: 
        end = 850+startyr+ntrain-1
#         else: 
#             end = 850+startyr+ntrain
        
        mod_filename_dict = {}
        for var in exp_setup['limvars']:
            if 'sic' in var: 
                mod_filename_dict[var] = ('_ntrunc'+str(nmodes_sic_list[0])+'_002_monthall_'+train_dsource+'_latcutoff_'+
                                          str(exp_setup['lat_cutoff'][var])+'_wtTrue_dtTrue_ntrain_'+str(850+startyr)+'_'+
                                          str(end)+'_20211014.pkl')
            else: 
        ...
        filename_end = (exp_setup['train_dsource']+'_002_ntrain'+'_'+exp_setup['mod_filename']['tas'][-22:-13]+
